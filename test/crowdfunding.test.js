...
// Update: deadline for this year-end
const yearEndDeadline = dateToUNIX("2025-12-31");
...
describe("Crowdfunding", () => {
  ...
  describe("Request for funding", async function () {
    it("Start a project", async function () {
      ...
      const deadline = yearEndDeadline;
      ...
      expect(event.events[0].args.projectContractAddress).to.equal(
        projectList[0]
      );
      expect(event.events[0].args.creator).to.equal(address1.address);
      expect(event.events[0].args.minContribution).to.equal(
        minimumContribution
      );
